...
const Mahasiswa = () => {
  ...
  // ✅ Sinkron total SKS dari kelas dan matakuliah
  const fetchMahasiswa = useCallback(async () => {
    ...
    try {
      ...
      const mahasiswaWithSKS = resMhs.data.map((mhs) => {
        const totalSKS = resKelas.data
          .filter((kls) => (kls.mahasiswa_ids || []).includes(mhs.id))
          .map((kls) => resMatkul.data.find((mk) => mk.id === kls.mata_kuliah_id)?.sks || 0)
          .reduce((a, b) => a + b, 0);

        return { ...mhs, totalSKS };
      });

      setMahasiswaList(mahasiswaWithSKS);
    } catch (err) {
      setError(err);
      showCustomToast("Gagal memuat data mahasiswa.", "error");
    ...
    return (
      <div className="flex justify-center items-center h-64">
        <p className="text-xl text-red-500">
          Error: Gagal memuat data. Silakan coba lagi. ({error.message})
        </p>
      </div>
    );
  }
                ...
                <tr key={mhs.id} className="even:bg-gray-100 odd:bg-white">
                  ...
                  <td className="py-2 px-4 text-center space-x-2">
                    <Button
                      ...
                      onClick={() =>
                        navigate(`/dashboard/mahasiswa/${mhs.id}`, { state: mhs })
                      }
                    >
                      Detail
                    </Button>
